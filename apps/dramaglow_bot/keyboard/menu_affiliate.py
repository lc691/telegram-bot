...
from ..handlers import referral_link_handler
from ..handlers import referral_stats_handler
    ...
    if message.chat.type in (ChatType.GROUP, ChatType.SUPERGROUP):
        ...
        try:
            await message.reply(
                ...
                "👉 <a href='https://t.me/dramaglow_bot?start=referral_menu'>Klik di sini untuk membuka</a>",
                parse_mode=ParseMode.HTML,
                disable_web_page_preview=True,
            )
        except Exception:
...
def register_referral(app: Client):
    ...
    app.add_handler(
        MessageHandler(
            referral_menu_entrypoint, filters.command(["referral", "r_menu", "r_link"])
        ),
        group=1,
    )
