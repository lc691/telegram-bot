...
from apps.caridrama_bot.usecases.inline_search.inline_search_flow import (
    run_inline_search_flow,
)
from apps.caridrama_bot.presenters.inline_search_presenter import InlineSearchPresenter
...
def register_inline_search(app: Client) -> None:
    ...
    async def inline_search_handler(_: Client, iq: InlineQuery):
        ...
        try:
            ...
            payload["next_offset"] = str(payload.get("next_offset") or "")
            payload.setdefault("cache_time", 5)
            ...
            log.debug(
                "[INLINE] ANSWER SKIPPED user=%s reason=QueryIdInvalid",
                user_id,
            )
